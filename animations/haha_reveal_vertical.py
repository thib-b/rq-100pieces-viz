#!/usr/bin/env python3
"""
"Haha" single — VERTICAL 9:16 — image-matched fluffy-bloom reveal → dissolve-to-photo (Blender headless).

This is the 9:16 (1152x2048) social-media variant of haha_reveal.py. It is a duplicate of the
landscape engine with only the aspect-ratio touch points changed (resolution, camera sensor_fit,
plate sizing) and a distinct mp4 output name; the bloom/reveal logic and RNG call order are
identical, so the picture is the same artwork, just reframed into a tall centered dish.
...
dish circle) is revealed underneath. From 3:01 to the 3:13 end it holds static on the plate photo.
  ...
  blender --background --python animations/haha_reveal_vertical.py -- --last [--output DIR]      # spot-check
  blender --background --python animations/haha_reveal_vertical.py -- --preview [--fullres] --output DIR
  blender --background --python animations/haha_reveal_vertical.py -- --output DIR               # full render (auto-encodes DIR/haha_reveal_vertical.mp4)
"""
...
RES_X, RES_Y = 1152, 2048                   # 9:16 vertical (portrait) for social media
ORTHO_SCALE = 10.0
...
def setup_camera():
    ...
    cam_data.type = 'ORTHO'
    cam_data.sensor_fit = 'HORIZONTAL'   # keep ortho_scale mapping to WIDTH in portrait (else AUTO fits height)
...
def encode_mp4(out_dir):
    ...
    if not (shutil.which('ffmpeg') or os.path.exists(ffmpeg)):
        ...
        return
    mp4 = os.path.join(out_dir, "haha_reveal_vertical.mp4")
...
def main():
    ...
    r_world = PLATE_FRAC * min(frame_w, frame_h) / 2.0   # fit dish to the SHORT dim (width in portrait)
